import { uploadProduct } from "@/app/_services/apiProducts";
import {
  Dialog,
  DialogClose,
  DialogContent,
...
const UploadProduct = () => {
  ...
  const handleDrop = (e) => {
    ...
    const validFiles = droppedFiles.filter((file) => file.type === "text/csv");
    if (validFiles.length !== droppedFiles.length) {
      toast.error("Only CSV files are allowed.");
    }
    setValue("csv_file", validFiles);
    generatePreviews(validFiles);
  };

  const handleFileChange = (e) => {
    const selectedFiles = Array.from(e.target.files);
    setValue("csv_file", selectedFiles);
  ...
  const generatePreviews = (files) => {
    const previewsArray = files.map((file) => file.name);
    setPreviews(previewsArray);
  };

  async function onSubmit({ csv_file }) {
  ...
  return (
    <Dialog open={open} onOpenChange={() => setOpen((open) => !open)}>
      <DialogTrigger className="btn-primary">Upload Products</DialogTrigger>
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <DialogHeader>
            ...
            >
              ...
              <h1 className="font-serif text-lg">Drop CSV File Here</h1>
