...
export async function deleteWarrantyForBulk(formData) {
  const token = localStorage.getItem("access-token");

  if (!token) return null;

  const { data } = await axios(`${BASE_URL}/warranty/bulk-action`, {
    method: "post",
    headers: {
      Authorization: `Bearer ${token}`,
    },
...
export async function updateWarrantyStatusForBulk(formData) {
  const token = localStorage.getItem("access-token");
